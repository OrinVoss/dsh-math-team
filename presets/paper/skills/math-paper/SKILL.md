...
| 角色 | 模型 | 用途与理由 |
|---|---|---|
| **主模型** | **Kimi K3**（`kimi-coding/k3`） | 主力：**世界知识丰富、敏锐度高** → 论文写作、证据检索与核验、知识判断、口径把握（本岗位的主要工作） |
| **编程/脚本** | **DeepSeek V4.1 Flash**（编程强） | 需要写脚本、编译/转换（LaTeX→PDF、DOCX 校验等）时，用 `workflow` 派 V4.1 Flash 子代理执行 |
| **审查（双模型各审一遍）** | **Kimi K3-256K**（`kimi-coding/k3-256k`）**+ DeepSeek V4.1 Flash** | 两个模型**各独立审一遍**：K3-256K 侧重知识/逻辑/引用/口径，V4.1 Flash 侧重代码/复现/格式实现；两份结论都要记录 |
...
- 识图/渲染审查另用**经济型视觉模型**（见「识图子代理」小节），不必用 K3/V4.1 Flash。
...
### 独立模型论文审阅（双模型各审一遍，对抗式评审）

为跳出主模型盲区，论文的**独立审阅/质检由两个模型各独立审一遍**——Kimi K3-256K 与 DeepSeek V4.1 Flash 交叉覆盖彼此盲区。不同厂商、不同能力侧重的模型能暴露主模型忽略的问题，如杜撰数值、口径不自洽、推荐前后矛盾、编造引用、代码附录不可复现。

- **两个审查模型各审一遍（强制）**：
  - **Kimi K3-256K**（`kimi-coding/k3-256k`）：侧重**知识、逻辑、引用、口径、结论与适用边界**（世界知识丰富、敏锐度高）；
  - **DeepSeek V4.1 Flash**：侧重**代码/附录、复现、格式实现与数值可追溯**（编程能力强）。
- 用 `workflow` 派发**两次**审查子代理，分别在 `agent(prompt, { provider, model })` 里指定这两个模型；**两份结论都要记录**（各自 PASS/FAIL + 问题清单）。
- **模型名不硬编码**：用 `llm` 服务探测本环境实际的 provider/model（本环境 `kimi-coding/k3-256k`、`opencode-go/deepseek-v4-flash` 等）。
...
- 例子（两个模型各派一次）：
  `agent('你是独立论文审稿人，审查 member-c 的论文……核对数值可溯源、口径一致、引用可核验；并针对每个核心模型攻击其假设，核对适用边界，按严重度给出通过/不通过。', { provider: 'kimi-coding', model: 'k3-256k' })`
  `agent('同上，重点审查代码/附录、复现与格式实现……', { provider: 'opencode-go', model: 'deepseek-v4-flash' })`
- **双模型结论合并**：任一方发现的问题都视为待修复项；按严重度修复后**必须重新派发该模型复审**（通用复验闭环），循环直至两个模型的审查都无遗留问题。主模型不得口头覆盖审查 FAIL，也不得修复后不经复审就宣称通过。任一方探测不到时如实标注"单模型审查，覆盖受限"。

...
- 例子（`provider`/`model` 用探测结果替换）：
  `agent('你是独立论文审稿人，用不同视角审查 member-c 的论文……核对数值可溯源、口径一致、引用可核验，按严重度给出问题并给出通过/不通过。', { provider: 'kimi-coding', model: 'k3-256k' })`
- 审查结果并入 `评审记录.md`，作为独立评审门禁证据；`不通过` 按严重度回退修正后复验。主模型不得口头覆盖审查 FAIL。

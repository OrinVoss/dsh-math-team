...
最后一条仅适用于 LaTeX/Markdown 经 Pandoc 转换得到的 DOCX。另需调用 `paper_format.validate_paper_structure()` 检查篇幅、公式/图/表数量、图表正文引用与参考文献对应关系，并把渲染后的实际页数传入 `rendered_pages`。随后抽检分页、公式、图表、页眉页脚和目录，并从实际 DOCX 提取文本，检查 `**`、反引号、标题 `#`、表格竖线、公式定界符 `$`/`$$`、行首列表标记和 `[待补充]` 等 Markdown 格式残留或占位符。命中项须逐条人工判断；非正文所需的残留必须从源文件清除、重新导出并复验。任何错误或无合理覆盖依据的预警都应修复后重新生成。
...
用户要求的全部格式冻结且各自确定性门禁全部返回 0 后，按 `../../../../references/Subagent调度.md` 派发独立质检 Subagent。质检者核对当届规则、主张—证据、数值与单位、图表编号和正文引用、Markdown 格式残留、支撑材料完整性及实际渲染效果；同时生成两种格式时再检查 Word/LaTeX 一致性。失败回执必须定位到页码、章节、命令或来源，并按根因返回对应角色；`W2` 未返回 `PASS` 不得交付。

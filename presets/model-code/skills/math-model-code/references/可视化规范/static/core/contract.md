> **⚠️ 工具脚本未随附**：本文档提及的绘图脚本（如 `setup_style.py`、`export_figure.py`、`visual_qa.py`、`apply_publication_style.m` 等）来自参考仓库的**工具脚本**，本预设**仅随附方法论文档，未包含这些脚本**。遇到此类脚本时按正文描述的方法自行实现等价脚本，或由团队另行接入；**不要假设它们存在于本机**。方法规范本身仍然有效。

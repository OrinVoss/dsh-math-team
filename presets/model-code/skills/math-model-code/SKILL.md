...
| 角色 | 模型 | 用途与理由 |
|---|---|---|
| **主模型** | **Kimi K3**（`kimi-coding/k3`） | 主力：参数量大、**世界知识丰富、敏锐度高** → 题目理解、建模分析、模型选型与方案判断等知识密集型工作 |
| **编程** | **DeepSeek V4.1 Flash**（编程能力强） | 编程/实现：开 `workflow` 派 **V4.1 Flash 子代理**写代码、跑结果、出图；它编程强但**知识领域可能有盲区**，故主要用于编码执行，不单独作知识权威 |
| **审查（双模型各审一遍）** | **Kimi K3-256K**（`kimi-coding/k3-256k`）**+ DeepSeek V4.1 Flash** | 两个模型**各独立审一遍**，交叉覆盖彼此盲区：K3-256K 侧重知识/逻辑/口径，V4.1 Flash 侧重代码/复现/实现正确性；两份结论都要记录 |

- 编程任务：用 `workflow` 的 `agent(prompt, { provider, model })` 指定 V4.1 Flash 执行（它编程强）。
- 审查任务：**必须双模型各审一遍**（K3-256K 一遍 + V4.1 Flash 一遍），两份结论都写入记录；任一模型发现的问题都要按通用复验闭环修复并复审。
- **模型名不硬编码**：换部署后用 `llm` 服务的 `listProviders()` / `resolveModelInfo()` 探测本环境实际的 provider/model（本环境 `kimi-coding/k3`、`kimi-coding/k3-256k`、`opencode-go/deepseek-v4-flash` 等）。
- 识图任务另用**经济型视觉模型**（见「识图子代理」小节），不必用 K3/V4.1 Flash。
...
为跳出主模型自身的盲区，交付物的**独立评审/质检由两个模型各独立审一遍**——Kimi K3-256K 与 DeepSeek V4.1 Flash 交叉覆盖彼此盲区。主模型容易对自己产出的内容失去批判距离，换成不同厂商、不同能力侧重的模型才能暴露它忽略的缺陷（如编造数值、口径不自洽、图表与结论脱节、代码不可复现）。

- **两个审查模型各审一遍（强制）**：
  - **Kimi K3-256K**（`kimi-coding/k3-256k`）：侧重**知识、逻辑、口径、结论合理性**与常识盲区（世界知识丰富、敏锐度高）；
  - **DeepSeek V4.1 Flash**：侧重**代码、复现、实现正确性与数值可追溯**（编程能力强）。
- 用 `workflow` 派发**两次**审查子代理，分别在 `agent(prompt, { provider, model })` 里指定这两个模型；**两份结论都要记录**（各自 PASS/FAIL 与问题清单）。
- **模型名不硬编码**：用 `llm` 服务的 `listProviders()` / `resolveModelInfo()` 探测本环境实际的 provider/model（本环境 `kimi-coding/k3-256k`、`opencode-go/deepseek-v4-flash` 等）。
- **审查 prompt 要求**（结构化，两次都要）：
  - 事实性：结论是否有真实结果/表/图支撑？有无编造的数值或来源？
  - 一致性：口径/符号/结论在报告、代码、结果、图之间是否自洽？
  - 完备性：是否覆盖全部子问题？有无遗漏的约束或维度？
  - 独立判断：基于以上给出 `通过 / 不通过`，不通过时列出必须修复的证据项。
- 例子（两个模型各派一次）：
  `agent('你是独立审查员，审查 member-a 的交付物（题目分析报告、代码、results、figures）……', { provider: 'kimi-coding', model: 'k3-256k' })`
  `agent('同上，重点审查代码与复现正确性……', { provider: 'opencode-go', model: 'deepseek-v4-flash' })`
- **双模型结论合并**：任一方发现的问题都视为待修复项；按证据修复后**必须重新派发该模型复审**（通用复验闭环），循环直至两个模型的审查都无遗留问题。任一方未审或探测不到时如实标注"单模型审查，覆盖受限"。

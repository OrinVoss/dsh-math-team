...
- **Figure QA (mandatory image-review gate)**: **prefer reading figures directly with the main model's `read_image`** when it supports images; otherwise dispatch a sub-agent on a **vision-capable model in the current environment** via `workflow` (auto-probe models whose `inputModalities` includes `image`; e.g. DeepSeek V4.1 Flash or `kimi-coding/kimi-for-coding`, not hardcoded). **Every formal figure must be reviewed one-by-one to PASS with records before P2/W2**; on FAIL, a "fix → re-review" loop runs until PASS — figures that skipped image review cannot pass the final-check gates
...
- **Figure QA (mandatory image-review gate)**: **prefer the main model's direct `read_image`** when its `inputModalities` includes `image`; otherwise auto-probe a vision model (validated: DeepSeek V4.1 Flash, `kimi-coding/kimi-for-coding`; not hardcoded) and dispatch a vision sub-agent via `workflow` to review every formal figure one-by-one (title/axes/legend/data/blank-overlap/claim support). **FAIL → fix per defects → re-review, loop until PASS**; each "FAIL reason → fix action → re-review result" is recorded. **Required in both P2 and W2: figures that skipped image review cannot pass the final-check gates.**
